...
1. 阅读 `sleep.c`，回答下列问题
   ...
   2. 请描述 `main` 函数参数 `argv` 中的指针指向了哪些字符串，他们的含义是什么。
      ...
      > 1. `argv[0] -> "sleep"`，`argv[0]` 应当指向调用了这个程序的 shell 所输入的程序名称，例如可以是 `sleep` 或是 `./sleep`、`/sleep` 等。
      > 2. `argv[1] -> "hello", argv[2] -> "world"`，`argv[1]` 及其之后的指针指向调用这个程序的 shell 所输入的参数。
...
2. 了解管道模型，回答下列问题
   ...
   2. fork之后，我们怎么用管道在父子进程传输数据？
      ...
      > 1. 首先关闭当前进程不需要使用的一端。
      > 2. 然后对写端使用 `write` 系统调用，对读端使用 `read` 系统调用。
