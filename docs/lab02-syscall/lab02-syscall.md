...
1. `kpagefree()`：还剩下多少内存页未分配。
   ...
   `kalloc.c` 中的内存管理是类似链表的内存管理，启动时将可分配的页以链表形式储存起来，需要申请内存时候取链表头即可。遍历一遍链表即可得到还剩多少内存页未分配，乘上页面大小即为剩余内存大小。

2. `procn()`：`state` 为 `UNUSED` 的进程有多少个。

   遍历 `proc[]`，`p->state == UNUSED` 的即为所求的进程。
